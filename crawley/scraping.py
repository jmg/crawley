...
class Document(Element):
    ...
    def __init__(self, html, url=None):
        self.raw_html = html
        root = lxml.html.fromstring(html) if html.strip() else lxml.html.Element("html")
        if url:
            try:
    ...
    def _extract_many(self, selector):
...
async def afetch(url, client=None, **kwargs):
...
async def afetch_all(urls, **kwargs):
