...
class ParserTest(unittest.TestCase):
    ...
    def _test_mas_de_una_propiedad(self):
        ...
        self.assertEquals("return [x for x in PyQuery(html).query('p')[0] if x in PyQuery(html).query('.mi-clase')[0]]", Parser().parse("first < tag:'p' class:'mi-clase' => innerHTML"))#, "pido la interseccion entre el primer tag p y la clase mi-clase")
